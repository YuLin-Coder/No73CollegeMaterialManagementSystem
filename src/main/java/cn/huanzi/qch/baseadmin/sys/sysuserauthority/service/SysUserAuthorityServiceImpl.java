...
@Service
@Transactional
public class SysUserAuthorityServiceImpl extends CommonServiceImpl<SysUserAuthorityVo, SysUserAuthority, Integer> implements SysUserAuthorityService{
    ...
    @Override
    public Result<List<SysUserAuthorityVo>> findByUserId(Integer userId) {
        return Result.of(CopyUtil.copyList(sysUserAuthorityRepository.findByUserId(userId),SysUserAuthorityVo.class));
    }

    @Override
    public Result<Boolean> saveAllByUserId(Integer userId, String authorityIdList) {
        //先删除旧的
        ...
        for (String authorityId : authorityIdList.split(",")) {
            sysUserAuthorityVo.setAuthorityId(Integer.parseInt(authorityId));
