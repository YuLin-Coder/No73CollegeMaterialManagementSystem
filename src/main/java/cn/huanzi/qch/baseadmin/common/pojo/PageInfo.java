...
public class PageInfo<M> {
     ...
     * 获取统一分页结果
     */
    public static <M> PageInfo<M> of(com.github.pagehelper.PageInfo page) {
        int records = (int) page.getTotal();
        int pageSize = page.getPageSize();
        int total = records % pageSize == 0 ? records / pageSize : records / pageSize + 1;

        PageInfo<M> pageInfo = new PageInfo<>();
        pageInfo.setPage(page.getPageNum());//页码
        pageInfo.setPageSize(pageSize);//页面大小
        pageInfo.setRows(page.getList());//分页结果
        pageInfo.setRecords(records);//总记录数
        pageInfo.setTotal(total);//总页数
        return pageInfo;
    }
     ...
     * 获取统一分页结果
     */
    public static <M> PageInfo<M> of(com.github.pagehelper.PageInfo page, Class<M> entityModelClass) {
        int records = (int) page.getTotal();
        int pageSize = page.getPageSize();
        int total = records % pageSize == 0 ? records / pageSize : records / pageSize + 1;
        ...
        pageInfo.setPage(page.getPageNum());//页码
        pageInfo.setPageSize(pageSize);//页面大小
        pageInfo.setRows(CopyUtil.copyList(page.getList(), entityModelClass));//分页结果
        pageInfo.setRecords(records);//总记录数
        pageInfo.setTotal(total);//总页数
        return pageInfo;
    }
