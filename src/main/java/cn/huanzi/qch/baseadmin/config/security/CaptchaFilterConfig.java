    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if ("POST".equals(request.getMethod()) && "/login".equals(request.getRequestURI().replaceFirst(contextPath,""))) {
            ...
            if(!StringUtils.isEmpty(sysUserVo.getUserId())) {
                ...
                authorityName = sysUserAuthorityVoList.get(0).getSysAuthority().getAuthorityName();
            }
            //查无此用户
            if(StringUtils.isEmpty(sysUserVo.getUserId()) || !authorityName.equals(role)){

                String dataString = "{\"code\":\"400\",\"msg\":\"用户名或密码错误\"}";
                //加密
                try {
