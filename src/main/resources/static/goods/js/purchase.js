...
layui.use(['element', 'form', 'table', 'layer', 'laydate', 'tree', 'util'], function () {
    ...
    tableIns = table.render({
        ...
        , cols: [[
            ...
            , {field: 'date', title: '购买日期'}
            , {field: 'supplierName', title: '供应商'}
            , {field: 'purchaser', title: '采购人'}
    ...
    table.on('toolbar(test)', function (obj) {
        switch (obj.event) {
            ...
            case 'query':
                ...
                //设定异步数据接口的额外参数
                query.where = {type:type,goodsName:goodsName};
    ...
    table.on('tool(test)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确认删除吗？', function (index) {
                //向服务端发送删除指令
                $.delete(ctx + "/goods/delete/" + data.id, {}, function (data) {
                    tableIns.reload();
                    layer.close(index);
                })
            });
...
function saveInstructor() {
    ...
    $.post(ctx + "/goods/save", instructorForm, function (data) {
        if(!data.flag){
